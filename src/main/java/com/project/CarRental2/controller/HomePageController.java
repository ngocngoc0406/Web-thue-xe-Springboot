...
public class HomePageController implements FiledName {
	...
	@Autowired
	private CarRecommendationService carRecommendationService;

	@Autowired
	private com.project.CarRental2.service.CarReviewService carReviewService;
	...
	public String booking(Model model, @RequestParam(name = "dateStart") String dateStart,
		...
		System.err.println("[BOOKING] Province: " + provinceId + ", District: " + districtId + ", Ward: " + wardId);

		HttpSession session = request.getSession();
		User user = (User) session.getAttribute("sesionUser");

		User sessionUser = (User) session.getAttribute("sesionUser");
		System.err.println("[BOOKING] Session User: " + sessionUser);
		if (user == null) {
			ra.addFlashAttribute("messege_error", "Chưa đăng nhập");
			return "redirect:/login";
		}
		if (sessionUser.getImgDrivingLicense() == null || sessionUser.getDrivingLicense() == null
				|| sessionUser.getPhone() == null) {
			ra.addFlashAttribute("messege_error", "Bạn chưa cập nhật bằng lái xe");
			return "redirect:/edit-profile";
		}

		// Lookup Province, District, Ward by IDs
		Province province = null;
		District district = null;
		Ward ward = null;
		String addressString = addressDetail;

		try {
			if (wardId != null && !wardId.equals("0") && !wardId.isEmpty()) {
				int wId = Integer.parseInt(wardId);
				ward = wardService.getAWard(wId);
				if (ward != null) {
					addressString = addressDetail + ", " + ward.getNameWard();
				}
			}
			if (districtId != null && !districtId.equals("0") && !districtId.isEmpty()) {
				int dId = Integer.parseInt(districtId);
				district = districtService.getDistrict(dId);
				if (district != null) {
					addressString = addressString + ", " + district.getNameDistrict();
				}
			}
			if (provinceId != null && !provinceId.equals("0") && !provinceId.isEmpty()) {
				int pId = Integer.parseInt(provinceId);
				province = provinceService.getProvince(pId);
				if (province != null) {
					addressString = addressString + ", " + province.getNameProvince();
				}
			}
		} catch (NumberFormatException e) {
			System.err.println("[BOOKING] Error parsing location IDs: " + e.getMessage());
		}

		System.err.println("[BOOKING] Final address: " + addressString);
		...
		booking.setAddress(addressString);
		System.err.println("[BOOKING] Booking object: " + booking.toString());
		System.err.println("[BOOKING] Car avatar: " + booking.getCar().getAvatarCar());
		...
		System.err.println("[BOOKING] Booking saved successfully!");
	...
	public String carDetail(Model model, @PathVariable(name = "id") int id_car,
			@PathVariable(name = "nameCar") String namCar, HttpServletRequest request) {
		...
		// Track user car view for AI recommendations
		HttpSession session = request.getSession();
		User sessionUser = (User) session.getAttribute("sesionUser");
		if (sessionUser != null) {
			try {
				carRecommendationService.trackCarView(sessionUser.getIdUser(), id_car);
			} catch (Exception e) {
				System.err.println("Error tracking car view: " + e.getMessage());
			}
		}

		// Add dynamic reviews data
		List<com.project.CarRental2.model.CarReview> listReviews = carReviewService.getReviewsByCarId(id_car);
		model.addAttribute("listReviews", listReviews);
		model.addAttribute("averageRating", carReviewService.getAverageRating(id_car));
		model.addAttribute("reviewCount", carReviewService.getReviewCount(id_car));
	...
	public String getContract(Model model, HttpServletRequest request) {
		...
		List<RequestWithdrawal> listRequestWithdrawals = requestWithdrawalService
				.getAllRequestWithdrawByIdUserOrderByCreateDate(user.getIdUser());
