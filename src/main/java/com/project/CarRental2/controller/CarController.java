...
public class CarController implements FiledName {
	...
	public String registerCar(@ModelAttribute("car") Car car,
			@RequestParam(name = "brandcar", required = false) String brandCarId,
			@RequestParam(name = "province", required = false) String provinceId,
			@RequestParam(name = "district", required = false) String districtId,
			@RequestParam(name = "ward", required = false) String wardId,
		...
		System.out.println("========== POST /register-car received ==========");
		System.out.println("brandCarId: " + brandCarId);
		...
		if (brandCarId == null || brandCarId.equals("0") ||
				provinceId == null || provinceId.equals("0") ||
				districtId == null || districtId.equals("0") ||
				wardId == null || wardId.equals("0")) {
			System.out.println("Validation failed - missing required fields");
			ra.addFlashAttribute("mes_faill", "Vui lòng chọn đầy đủ hãng xe, tỉnh/thành phố, quận/huyện và phường/xã");
			return "redirect:/register-car";
		}

		// Lookup entities
		BrandCar brandCar = brandCarService.getBrandCarById(Integer.parseInt(brandCarId));
		Province province = provinceService.getProvince(Integer.parseInt(provinceId));
		District district = districtRepository.findById(Integer.parseInt(districtId)).orElse(null);
		Ward ward = wardRepository.findById(Integer.parseInt(wardId)).orElse(null);

		if (brandCar == null || province == null || district == null || ward == null) {
			System.out.println("Entity lookup failed");
			ra.addFlashAttribute("mes_faill", "Dữ liệu không hợp lệ");
			return "redirect:/register-car";
		}
	...
	public String saveCarEdit(@ModelAttribute("car") Car car, @RequestParam("brandcar") BrandCar brandCar,
		...
		if (sessionUser != null) {
			if (sessionUser.getRole().getIdRole() == ROLE_CUSTOMMER_CARE
				...
				car.setAddressCar(
						car.getAddressCar() + "," + ward.getNameWard() + "," + district.getNameDistrict() + ","
								+ province.getNameProvince());
				car.setBrandCar(brandCar);
				car.setCreateDate(new Date());
				car.setUpdateDate(new Date());
	...
	public String deleteCar(@PathVariable(name = "id") int idCar, HttpServletRequest request, RedirectAttributes ra) {
		HttpSession session = request.getSession();
		User sessionUser = (User) session.getAttribute("sesionUser");
		if (sessionUser != null) {
			if (sessionUser.getRole().getIdRole() == ROLE_CUSTOMMER_CARE
					|| sessionUser.getRole().getIdRole() == ROLE_ADMIN) {
				try {
					carService.deleteACarById(idCar);
					ra.addFlashAttribute("mes_success", "Xóa xe thành công");
				} catch (Exception e) {
					System.err.println("Error deleting car: " + e.getMessage());
					ra.addFlashAttribute("mes_faill", "Không thể xóa xe. Xe có thể đang được sử dụng.");
				}
				return "redirect:/admin/car";
			} else {
				return "redirect:/login";
