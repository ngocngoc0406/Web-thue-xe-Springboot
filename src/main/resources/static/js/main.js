...
$(document).ready(function() {
	...
	if (inputDateStart) inputDateStart.value = datetimeStringStart;
	if (inputDateEnd) inputDateEnd.value = datetimeStringEnd;
	...
	var inputTotalBillEl = document.getElementById("input_total_bill");
	
	if (priceBookingEl && priceBhEl && priceDvEl && priceTotalEl && totalBillEl && inputTotalBillEl) {
		var pricebooking = priceBookingEl.innerText;
		var pricebh = priceBhEl.innerText;
		var arrpricebooking = splitData("K", pricebooking);
		var price_dv_by_price = (Number(arrpricebooking[0]*1000)*5/100+ 1000);
		priceDvEl.innerText = price_dv_by_price.toString().substring(0,price_dv_by_price.toString().length-3)+"K/ngày";
		var pricedv = priceDvEl.innerText;
